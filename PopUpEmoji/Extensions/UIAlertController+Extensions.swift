...
extension UIAlertController {
    ...
    func addEmojiPicker(flow: UICollectionView.ScrollDirection, paging: Bool, selection: @escaping Selection) {
        let vc = EmojiPopUpViewController(flow: flow, paging: paging, selection: selection)
        
        if UIDevice.current.userInterfaceIdiom == .pad {
            vc.preferredContentSize.height = App.EmojiPopUpViewController.PreferredSize.height * 0.9
            vc.preferredContentSize.width = App.EmojiPopUpViewController.PreferredSize.width * 0.9
        } else {
            vc.preferredContentSize.height = App.EmojiPopUpViewController.PreferredSize.width * 0.9
        }
        set(vc: vc)
    }
}
